...
function server {
  ...
  chown $user:$user src/ecc_key.der
...
function client {
  ...
  if [ -f /.dockerenv ]; then #testing if we are on docker
      /etc/init.d/dbus stop
      /etc/init.d/dbus start
      sleep 2
      /etc/init.d/avahi-daemon stop
      /etc/init.d/avahi-daemon start;
      path='/root' #root is the folder for docker
  fi
  pushd .
  cd ../..
  make mesh_tb_client
  ...
  if [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]]; then
    ...
    if [[ ${path} != *"root"* ]];then #not in docker
     # testmystring does not contain c0
     read -p "- Server Username: " server_user
     path='/home/$server_user'
    fi
    # pull the key from the server
    scp $server_user@$server_ip:$path/mesh_com/modules/sc-mesh-secure-deployment/src/ecc_key.der src/ecc_key.der
  fi
